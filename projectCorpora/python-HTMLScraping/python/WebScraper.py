# 2022-02-28 ebb: Updating this to scrape scripts from the Avatar wiki at https://avatar.fandom.com/wiki/Category:Avatar:_The_Last_Airbender_episode_transcripts
...
archive_url = 'https://avatar.fandom.com/wiki/Category:Avatar:_The_Last_Airbender_episode_transcripts'
...
def get_files():
    ...
    links = [archive_url.split(archiveFileName)[0] + link.split('/wiki/')[1] for link in root.xpath('//a[contains(@title, "Transcript")]/@href')]
...
def download_links(links):
    for link in links:
        ...
        # ebb: file path
        path = os.path.join("avatarTranscripts", file_name)
        # download started
