...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- Added priority to avoid some error messages I was getting
    Tried to locate beginnings of <sp> tags, ending when a ':' appeared.
    -->
    <xsl:template match="text()"  priority="2"> 
        <xsl:analyze-string select="." regex="&lt;sp&gt;(.*):">
           
            <xsl:matching-substring>
                <speakTag><xsl:value-of select="regex-group(1)"/></speakTag>
            </xsl:matching-substring>
