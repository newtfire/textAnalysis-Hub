...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <svg width="100%" height="600">
            ...
            <line x1="50" y1="550" x2="950" y2="550" stroke="darkpurple" stroke-width="2"/>
            <line x1="50" y1="550" x2="50" y2="50" stroke="darkpurple" stroke-width="2"/>
            ...
            <g id="grid">
                <xsl:for-each select="1 to 10">
                    <line x1="{50 + position() * $x-spacer}" y1="50"
                        x2="{50 + position() * $x-spacer}" y2="550" stroke="darkblue"
                        stroke-width="0.5"/>
                    <line x1="50" y1="{550 - position() * $y-spacer}" x2="950"
                        y2="{550 - position() * $y-spacer}" stroke="darkgreen" stroke-width="0.5"/>
                </xsl:for-each>
            </g>
            ...
            <g transform="translate(20 500)">
                ...
                <xsl:for-each select="descendant::blob">
                    ...
                    <circle cx="{50 + position() * $x-spacer}" cy="{550 - (number(@value) * 5)}"
                        ...
                        <animate attributeName="r"
                            from="{count(descendant::special[@whatsIt='magicEgg']) * 5}"
                            to="{count(descendant::special[@whatsIt='magicEgg']) * 8}" dur="1s"
                            repeatCount="indefinite" begin="0s" alternate="true"/>
                    </circle>
                    <text x="{50 + position() * $x-spacer}" y="{550 - (number(@value) * 5)}" 
                        font-size="12" text-anchor="middle" fill="lightpurple">
                ...
                <xsl:for-each select="descendant::node">
                    ...
                    <rect x="{50 + position() * $x-spacer}" y="{550 - (@height * $y-spacer)}"
                        width="40" height="{@height * $y-spacer}" fill="green" stroke="darkgreen"
                        stroke-width="2"/>
                </xsl:for-each>
            </g>
        </svg>
